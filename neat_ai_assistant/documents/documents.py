...
class Document:
    ...
    def add_embedding_for_chunk(
        self, chunk_idx: int, embedding: Sequence[float]
    ) -> None:
...
class DocumentSearchResult:
    ...
    def add_best_chunks(self, threshold: float) -> None:
        ...
        for idx, score in enumerate(self.chunk_scores):
            if score > threshold:
                best_chunks.append((self.document.chunks[idx], score))
        self.best_chunks = sorted(
            best_chunks, key=lambda chunk_score: chunk_score[1], reverse=True
        )


class DocumentMinion:
    def __init__(self, llm_wrapper: LLMWrapper, documents_path: Path) -> None:
    ...
    def get_description(self) -> None:
        document_text_excerpts = "\n".join(d.get_excerpt() for d in self.documents[:10])
...
```""",
    ...
    def _generate_hash_id(
        file_path: Optional[Path] = None, string: Optional[str] = None
    ) -> str:
    ...
    def _create_document_object_from_file(
        self, file_path: Path, doc_hash_id: str
    ) -> Document:
        raw_chunks = self.parser.parse_file(file_path)
        return Document(
            chunks=[
                Chunk(content=c, hash_id=self._generate_hash_id(string=c))
                for c in raw_chunks
            ],
            hash_id=doc_hash_id,
            path=file_path,
    ...
    def _add_document_embeddings(self, document: Document) -> None:
        request_dicts = [
            self.llm_wrapper.build_aleph_alpha_request(
                request_object=SemanticEmbeddingRequest(
                    prompt=Prompt.from_text(text=c.content),
        ...
        responses: Sequence[
            SemanticEmbeddingResponse
        ] = self.llm_wrapper.aleph_alpha_batch_request(
            requests=request_dicts,
        )
        for idx, r in enumerate(responses):
            document.add_embedding_for_chunk(chunk_idx=idx, embedding=r.embedding)
    ...
    def _document_from_raw(self, document: dict, id: str) -> Document:
        return Document(
            chunks=[Chunk(**c) for c in document["chunks"]],
            hash_id=id,
            path=document["path"],
        )
        ...
        for file_path in tqdm(file_paths, desc="Parsing and embedding..."):
            ...
            if doc_hash_id not in saved_documents:
                ...
                saved_document = self._document_from_raw(
                    document=raw_saved_document, id=doc_hash_id
                )
                documents.append(saved_document)
        return documents

    def _save_documents(
        self, documents: Sequence[Document], max_documents_per_file: int = 50
    ) -> None:
        for i in range(0, len(documents), max_documents_per_file):
            ...
            save_path = (
                self.database_path / f"documents{i//max_documents_per_file}.json"
            )
    ...
    def instantiate_database(self, update: bool = True) -> None:
        if update:
            ...
            documents = [
                self._document_from_raw(d, key) for key, d in raw_documents.items()
            ]
        self.documents = documents
        self.get_description()

    def score_chunks(
        self, embedded_query: Sequence[float], threshold: float
    ) -> Sequence[DocumentSearchResult]:
        results = []
        for d in self.documents:
            chunk_scores = [
                self.llm_wrapper.compute_cosine_similarity(
                    embedding_1=embedded_query, embedding_2=c.embedding
                )
                for c in d.chunks
            ]
            res = DocumentSearchResult(document=d, chunk_scores=chunk_scores)
    ...
    def search(
        ...
        request_dict = self.llm_wrapper.build_aleph_alpha_request(
            request_object=SemanticEmbeddingRequest(
                prompt=Prompt.from_text(text=question),
        ...
        results = self.score_chunks(
            embedded_query=response.embedding, threshold=threshold
        )
        return results
