...
class DocumentSearchTool(Tool):
    ...
    @classmethod
    def from_document_minion(
        cls, document_minion: DocumentMinion
    ) -> "DocumentSearchTool":
        return cls(
    ...
    def run(self, json_query: dict) -> ToolResult:
        ...
        formatted_results = (
            [r.best_chunks[0][0].content for r in results[: json_query["n"]]]
            if bool(results)
            else []
        )
        # Currently returns only best chunk per document -> desired?
        return self._build_tool_result(formatted_results)
